...
#Acceptable combos, rotating clockwise each increment
accComb = {
  "x" :["x","x","x"],
  "st":["rt","","lt"],
  "lt":["st","rt",""],
...
def main():
  ...
  for item in sorted(maze["n"]):
    #Compare value for each square to ensure each perspective agrees
    check = [maze["n"][item],maze["e"][item],maze["s"][item],maze["w"][item]]
    
    checked = 0

    for val in range(4):
      if check[val] in accComb:
        checked  = 1
        rearr = [ check[ val % 4 ],
                  check[ (val + 1) % 4 ],
                  check[ (val + 2) % 4 ],
        ...
        if doCheck(rearr):
          ...
          print("Error on square " + item + "; Rot: " + str(val) + "; " + str(check))

    if not checked:
      print("No valid values in square " + item)

  print("Done")

def doCheck(arr):
  if arr[1] == accComb[arr[0]][0] and arr[2] == accComb[arr[0]][1] and arr[3] == accComb[arr[0]][2]:
    return 1
  else:
    return 0
